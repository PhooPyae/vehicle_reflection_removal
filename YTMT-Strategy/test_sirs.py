...
# datadir = os.path.join(os.path.expanduser('~'), 'test')
datadir = os.path.join('/root/lily/cv/panel_detection/carro-ds-cv/detectron2/YTMT-Strategy/test')
...
# eval_dataset_cars = datasets.RealDataset(join(datadir, 'cars'))
...
# # eval_dataset_postcard = datasets.CEILTestDataset(join(datadir, 'cars'),
# #                                                  if_align=opt.if_align)
...
test_dataloader_cars = datasets.DataLoader(
    eval_dataset_cars, batch_size=1, shuffle=True,
    num_workers=opt.nThreads, pin_memory=True)
...
#
res = engine.test(test_dataloader_cars, savedir=join(result_dir, 'cars'))
print(res)
