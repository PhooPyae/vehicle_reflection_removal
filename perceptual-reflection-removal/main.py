...
def build(input):
    ...
    if hyper:
        ...
        vgg19_features=build_vgg19(input[:,:,:,0:3]*255.0)
        # print('vgg19_features: {}'.format(len(vgg19_features)))
        for layer_id in range(1,6):
            vgg19_f = vgg19_features['conv%d_2'%layer_id]
...
# To test the model on images with reflection
    ...
    test_path = ["./test_images/cars/"] #["./test_images/real/"]
    subtask="cars" # if you want to save different testset separately
    ...
    for val_path in val_names:
        ...
        print('{} : img shape {}'.format(val_path, img.shape))
